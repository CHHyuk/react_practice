...
// useEffect의 [] : 의존성 / 안의 변수가 변경될때마다 실행
  useEffect(() => {
  ...
  return (
    <>
      <div style={{ border: "10px solid blue", padding: 10 }}>
        appNo : {appNo}
        <button className="btn btn-outline" onClick={() => setNo(no + 1)}>Sub 버튼(no) : {no}</button>
        <button className="btn btn-outline" onClick={() => setNo2(no2 + 1)}>Sub 버튼(no2) : {no2}</button>
      </div>
    </>
  );
}
